...
export default function ProfileModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm rounded-2xl border border-zinc-700 bg-zinc-900 p-6 shadow-2xl overflow-hidden">
        <button className="absolute right-4 top-4 text-zinc-500 hover:text-zinc-300" onClick={onClose}>✕</button>

        <div className="mb-5">
          {section && (
            <button className="mb-3 text-xs font-semibold text-zinc-500 hover:text-zinc-300 transition-colors" onClick={() => { setSection(null); reset(); }}>
              ← Back
            </button>
          )}
        ...
        {!section ? (
          <div className="space-y-2">
            {sections.map(({ key, label, description }) => (
              <button
                key={key}
                className="w-full flex items-center justify-between rounded-xl border border-zinc-800 bg-zinc-950/60 px-4 py-3 text-left hover:border-zinc-700 hover:bg-zinc-800/60 transition-colors"
          ...
          <div className="space-y-3">
            ...
            {section === "birthday" && (() => {
              const daysInMonth = bdMonth && bdYear
                ? new Date(Number(bdYear), Number(bdMonth), 0).getDate()
                : 31;
              // Reset day if it exceeds the valid range
            ...
            {success && (
              <p className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-300">{success}</p>
            )}
